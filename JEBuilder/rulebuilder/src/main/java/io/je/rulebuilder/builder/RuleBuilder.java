...
import io.je.rulebuilder.components.GetterBlockSummary;
...
public class RuleBuilder {
	...
	static boolean eliminateCombinatoryBehaviour;
	...
	public static void buildRule(JERule jeRule, String buildPath, boolean compileOnly)
		...
		String rulePath = "";
		JELogger.control(JEMessages.BUILDING_RULE + " : " + jeRule.getJobEngineElementName(), LogCategory.DESIGN_MODE,
				jeRule.getJobEngineProjectID(), LogSubModule.RULE, jeRule.getJobEngineElementID());
	 ...
	 */
	public static void sendDRLToJeRunner(JERule rule, String path, boolean compileOnly)
			throws RuleBuildFailedException, JERunnerErrorException {
		...
		JELogger.debug(
				" [ project = " + rule.getJobEngineProjectName() + " ] [rule = " + rule.getJobEngineElementName() + "]"
		...
		JELogger.debug(" Rule : " + ruleMap, LogCategory.DESIGN_MODE, rule.getJobEngineProjectID(), LogSubModule.RULE,
		...
		if (compileOnly) {
			...
			try {
				...
				JELogger.error(
						" [ project = " + rule.getJobEngineProjectName() + " ] [rule = "
								+ rule.getJobEngineElementName() + "]" + JEMessages.RULE_BUILD_FAILED,
						LogCategory.DESIGN_MODE, rule.getJobEngineProjectID(), LogSubModule.RULE,
						rule.getJobEngineElementID());
	...
	public static List<ScriptedRule> scriptRule(UserDefinedRule uRule) throws RuleBuildFailedException {
		...
		for (Block root : rootBlocks) {
			...
			{
				//Map<String, List<Block>> allGetterBlocksByClassId = getAllGetterBlocksByClassId(root,new HashMap<>());
				//eliminateCombinatoryBehaviour(allGetterBlocksByClassId);
			}
			...
			String script = generateScript(uRule.getRuleParameters(), scriptedRuleid, duration, condition,
					consequences);
			JELogger.debug(JEMessages.GENERATED_RULE + script, LogCategory.DESIGN_MODE, uRule.getJobEngineProjectID(),
	...
	private static void eliminateCombinatoryBehaviour(GetterBlockSummary allGetterBlocksByClassId) {
	 ...
	 */
	private static GetterBlockSummary getAllGetterBlocksByClassId(Block root,
			GetterBlockSummary allGetterBlocksByClassId) {
		for (Block input : root.getInputBlocks()) {
			if (input instanceof AttributeGetterBlock) {
				...
				allGetterBlocksByClassId.addBlock(getter);
			}else
			{
				allGetterBlocksByClassId = getAllGetterBlocksByClassId(input,allGetterBlocksByClassId);
			}
		}
	...
	private static String generateScript(RuleParameters ruleParameters, String ruleId, String duration,
			String condition, String consequences) throws RuleBuildFailedException {
		...
		if (ruleParameters.getDateEffective() != null && !ruleParameters.getDateEffective().isEmpty()) {
			LocalDateTime date = LocalDateTime.ofInstant(Instant.parse(ruleParameters.getDateEffective()),
					ZoneId.systemDefault());
			ruleTemplateAttributes.put("dateEffective",
					"\"" + DateUtils.formatDate(date, ConfigurationConstants.DROOLS_DATE_FORMAT) + "\"");
		}
		if (ruleParameters.getDateExpires() != null && !ruleParameters.getDateExpires().isEmpty()) {
			LocalDateTime date = LocalDateTime.ofInstant(Instant.parse(ruleParameters.getDateExpires()),
					ZoneId.systemDefault());
			ruleTemplateAttributes.put("dateExpires",
					"\"" + DateUtils.formatDate(date, ConfigurationConstants.DROOLS_DATE_FORMAT) + "\"");
		}
	...
	public static Set<Block> getRootBlocks(UserDefinedRule uRule) throws RuleBuildFailedException {
		...
		if (executionBlockCounter == 0) {
			JELogger.error(
					"[project = " + uRule.getJobEngineProjectName() + "][rule = " + uRule.getJobEngineElementName()
							+ "] " + JEMessages.NO_EXECUTION_BLOCK,
					LogCategory.DESIGN_MODE, uRule.getJobEngineProjectID(), LogSubModule.RULE,
					uRule.getJobEngineElementID());
